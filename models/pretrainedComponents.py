...
# Embedding is 3,256,256 -> 32,32,32
class FluxVAEEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        vae = AutoencoderKL.from_pretrained(
            "black-forest-labs/FLUX.2-klein-9B", subfolder="vae"
        )
    ...
    ) -> Float[torch.Tensor, "b vaeChannel heightVae widthVae"]:
...
class FluxVAEDecoder(nn.Module):
    ...
    @jaxtyped(typechecker=beartype)
    def forward(
        self, x: Float[torch.Tensor, "b latentChannels heightVae widthVae"]
